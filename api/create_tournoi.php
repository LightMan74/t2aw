...
// Capture toute sortie parasite (warnings HTML, etc.)
ob_start();
...
$champsRequis = ['nom_tournoi', 'nbre_terrain_poule', 'nbre_terrain_phasefinal', 
                  'temps_de_match', 'heure_debut_poule', 'heure_debut_phasefinal', 'categories'];
...
try {
    ...
    $idTournoi = (int) $stmt->fetch(PDO::FETCH_ASSOC)['next_id'];
    ...
    $stmt = $pdo->prepare("
        ...
        (id_tournoi, nbre_terrain_poule, nbre_terrain_phasefinal, temps_de_match, heure_debut_poule, heure_debut_phasefinal)
        VALUES (?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([
        $idTournoi,
